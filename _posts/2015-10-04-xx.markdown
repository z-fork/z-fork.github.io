...
IP 层发送 Packet 时, 若一个 Packet 的大小受 MTU 限制, 就会被分成若干个小于 MTU 的报文, 每个 Packet 都会有*独立的 IP 头部*

TCP 层在以太网中发送一个大于 1500 字节的消息, 调用 IP 层方法发送消息时, IP 层会自动的获取所在网络的 MTU, 并按照所在网络的 MTU 大小来分片

IP 层希望分片对于 TCP 层来说是透明的, 接收方的 IP 层会根据收到的多个 IP 包头部, 将分片的 Packet 重组为一个 Segment
